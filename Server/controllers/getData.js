...
exports.getAllTransactions = (req, res, next) => {
  Transaction.find()
    .populate("hotel", "name-_id")
    .then((data) => {
      // Sort trasactions by most recent booking date
      const sortedData = data.sort((a, b) => b.dateStart - a.dateStart);
      res.status(200).json(sortedData);
      // res.status(200).json([]);
    })
    .catch((err) => {
      console.log("Error in get all transaction data function: ", err);
      res.send({ message: err });
    });
};
